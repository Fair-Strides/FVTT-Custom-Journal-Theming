import { VideoPopout } from './video-popout.js';

export class VideoJournal extends DocumentSheet {
	static get defaultOptions() {
		return foundry.utils.mergeObject(super.defaultOptions, {
			baseApplication: 'JournalSheet',
			classes: [],
			template: 'modules/custom-journal/templates/customvideo.html',
			closeOnSubmit: false,
			submitOnClose: true,
...
//		await this.setDefaultFlag('image_rotation', '0');
	...
	async _render(force, options = {}) {
		let reset = false;
		if (!this.getFlag('video_position')) reset = true;
...
//			imgRotation: this.getFlag('image_rotation'),
		...
		setTimeout(() => {
			html.find('.custom-video-buttons a').click((event) => {
	...
	_getHeaderButtons() {
		...
		if (this.options.editable && canConfigure) {
			...
			buttons.push({
				...
				icon: 'fas fa-magic',
				onclick: (ev) => this._onConfigVideo(ev),
			});
			buttons.push({
				label: 'Unlock',
				class: 'lock-sheet',
...
//		const imageRotation = this.getFlag('image_rotation');
		...
		const html = await getTemplate('modules/custom-journal/templates/customvideo-config.html');
...
class DraggableElement extends Draggable {
	...
	_onRotateMouseDown(event) {
		...
		console.log("rotation at start: ", this.rotation);
		console.log("position:", event.clientX);
		const pos = this.element.getBoundingClientRect();
		this._initial = { x: pos.x + pos.width / 2, y: pos.y + pos.height / 2 };
		this._initialClient = { x: event.clientX, y: event.clientY};
		console.log("position at start: ", this._initial);
	...
	_onRotateMouseMove(event) {
		event.preventDefault();
console.log("Initial X: ", this._initialClient.x, "client X: ", event.clientX);
console.log("Initial Y: ", this._initialClient.y, "client Y: ", event.clientY);
...
//		const diffY = (this._initial.y - event.clientY) / this.position.height;
		const diffX = (this._initialClient.x - event.clientX);// / this.position.width;
		const diffY = (this._initialClient.y - event.clientY);// / this.position.height;
		const tan = diffY / diffX;

//		let atan = (Math.atan(tan) * 180) / Math.PI;
		let atan = (Math.atan2((this._initialClient.x - event.clientX), (this._initialClient.y - event.clientY)) * 180) / Math.PI;
		console.log("diffX", diffX);
		console.log("diffY", diffY);
		console.log("tan", tan);
		console.log("Atan 1", atan);
